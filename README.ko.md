...
재전송 불필요)이 열린다. 이 셋은 요청-응답 한 번으로는 구조적으로
성립하지 않는다.

권한 브로커링(`session/request_permission`을 정적 신뢰 목록이 아니라
Claude Code 쪽 판단으로 중재)은 ACP가 여는 네 번째 항목이고 transport에
역방향 요청 경로까지 구현되어 있으나, **아직 결정 핸들러를 배선한 커맨드가
없어 현재는 모든 요청이 자동 거부된다.** 에이전트가 필요한 읽기 툴을
미리 신뢰하므로(ADR-002) 지금 제공하는 플로우는 여기에 의존하지 않는다.
...
macOS, kiro-cli 2.20.2(2026-09-01)와 2.21.0(2026-09-04). 2.21.0에서 subprocess
폴백의 엔진 기본값과 stream-json 봉투 형태가 바뀌었으나 양쪽 모두 처리하므로
어느 버전에서도 동작한다.
